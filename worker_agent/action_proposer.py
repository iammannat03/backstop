...
async def propose_action(
    ticket_id: uuid.UUID, raw_action: ProposedAction, stripe_history: StripeHistory
) -> ProposedAction:
    ...
    with SessionLocal() as db:
        ...
        ticket.proposed_action = validated.model_dump(mode="json")
        ticket.status = "proposed_action"
        if stripe_history.customer_id:
            ticket.customer_id = stripe_history.customer_id
        db.add(
            AuditRecord(
                ticket_id=ticket_id,
                event_type="proposed_action",
                actor="worker_agent",
                detail=validated.model_dump(mode="json"),
            )
        )
        db.commit()
